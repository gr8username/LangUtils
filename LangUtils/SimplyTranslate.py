...
simply_response = None


def simply_translate_data(source_lang, target_lang, text): # TODO: add caching
    global simply_response
    simply_response = None
    # There's almost certainly a better way to do this... but this is simple and easy to understand
    thread = Thread(target=simply_translate_data_nocache, args=(source_lang, target_lang, text))
    thread.start()
    time_waited = 0
    while simply_response is None:
        time.sleep(1)
        time_waited += 1
        if time_waited > 10:
            return None

    return simply_response
...
def simply_translate_data_nocache(source_lang, target_lang, text):
    ...
    simply_response = simply_extract_result(response)
